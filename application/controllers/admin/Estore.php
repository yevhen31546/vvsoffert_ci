...
class Estore extends CI_Controller {
    ...
    public function create() {
        ...
        if (empty($_FILES['logoFile']['name'])) {
            $this->form_validation->set_rules('logoFile', 'Logo', 'required');
        }
        ...
        //if (isset($_POST) && count($_POST) > 0) {
            ...
            if ($submit == "save") {*/
                ...
                if (empty($_FILES['logoFile']['name'])) {
                    $this->form_validation->set_rules('logoFile', 'E-Store Logo', 'required');
                }
                ...
                if ($this->form_validation->run() === TRUE) {
                    ...
                    $new_name = $newStore["name"] . '_' . time();
                    $config = array(
                        'upload_path' => './images/estore',
    ...
    //                    'allowed_types' => "gif|jpg|png|jpeg|pdf",
                    ...
                    if (!$this->upload->do_upload('logoFile')) {
                        ...
                        $this->form_validation->set_message('logoFile', $this->upload->display_errors());
                    } else {
                        $data = array('upload_data' => $this->upload->data());
                    }

                    $newStore["logoImage"] = 'images/estore/'. $data['upload_data']['file_name'];
                    ...
                    $session_message['content'] = $this->form_validation->error_array();
                    $this->session->set_flashdata('message', $session_message);
*/
        ...
        if ($this->input->is_ajax_request()) {
            ...
            if ($this->form_validation->run() === TRUE) {
                ...
                if($_FILES['logoFile']['name'] != '') {
                    ...
                    $new_name = $updateStore["name"] . '_' . time();
    ...
    //                    'allowed_types' => "gif|jpg|png|jpeg|pdf",
                    ...
                    if (!$this->upload->do_upload('logoFile')) {
                        ...
                        $error['logoFile'] = strip_tags($this->upload->display_errors());

                    } else {
                        $data = array('upload_data' => $this->upload->data());
                    }

                    $updateStore["logoImage"] = 'images/estore/'. $data['upload_data']['file_name'];
    ...
    public function mail_excel($storeId = 0) {
        ...
        set_page_title(ucwords($findStore->name) . ": Send Excel File By Email");

        $this->data["storeInfo"] = $findStore;


        $data['content'] = $this->load->view('admin/estore/e-store-mail-excel', $this->data, TRUE);
        $this->load->view('layouts/admin', $data);
                                ...
                                foreach ($Spreadsheet as $Key => $Row) {
                                    if (trim($Row[0]) != "" && trim($Row[1]) != "") {
                                        if ($Key == 0) {
                                            ...
                                            } elseif ($importType == 4 && strcasecmp(strtolower(str_replace(" ", "", trim($Row[0]))), strtolower("RSKnumber")) == 0 && strcasecmp(strtolower(str_replace(" ", "", trim($Row[1]))), strtolower("E-StoreLink")) == 0) {
                                                
                                            }elseif ($importType == 5 && strcasecmp(strtolower(str_replace(" ", "", trim($Row[0]))), strtolower("RSKnumber")) == 0 && strcasecmp(strtolower(str_replace(" ", "", trim($Row[1]))), strtolower("Instock")) == 0) {
                                            ...
                                            if ($importType == 1) {
                                                ...
                                                $product_name = str_replace("'", "", trim($Row[1]));
                                                ...
                                                $e_store_link = trim($Row[1]);
                                                $insertSql = "INSERT INTO estore_products (id, track_id, store_id, rsk_no,e_store_link,created_at,updated_at) VALUES (NULL, '$trackId', '$store_id', '$rsk','$e_store_link','$currentDate','$currentDate') ON DUPLICATE KEY UPDATE e_store_link='$e_store_link',updated_at='$currentDate'";
                                            }
                                             elseif ($importType == 5) {
                                                $instock = trim($Row[1]);
                                                $insertSql = "INSERT INTO estore_products (id, track_id, store_id, rsk_no,in_stock,created_at,updated_at) VALUES (NULL, '$trackId', '$store_id', '$rsk','$instock','$currentDate','$currentDate') ON DUPLICATE KEY UPDATE in_stock='$instock',updated_at='$currentDate'";
                                            }
...
//                                $uploadFile = true;
                                foreach ($Spreadsheet as $Key => $Row) {
                                    if ($Key > 0) {
                                        if (trim($Row[0]) != "") {
                                            ...
                                            $e_store_link = isset($Row[7]) ? addslashes(trim($Row[7])) : "";
                                            $productArray[] = "(NULL, '$trackId', '$store_id', '$rsk_no', '$price', '$discount_price','$discount_group','$unit','$in_stock','$product_name','$currentDate','$currentDate','$e_store_link')";
                                            if (count($productArray) == 500) {
                                                $count1 = $count1 + 500;
                                                $proQuery = 'INSERT IGNORE INTO estore_products (id, track_id, store_id, rsk_no, price,discountprice,discount_group, unit,in_stock,product_name,created_at,updated_at,e_store_link) VALUES ';
                                ...
                                if ($error == false && !empty($productArray)) {
...
//                            echo $E->getMessage();
    ...
    public function uept() {
        ...
        $this->dbforge->add_column('estore_products', $fields);
    ...
    public function uet() {
        ...
        $this->dbforge->add_column('estore_master', $fields);
