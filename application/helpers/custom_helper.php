...
function lookupProduct2($keyword,$limit=10,$start=0){ 
        $ci = & get_instance();
        $ci->db->select('pro.*'); 
        $ci->db->from('products as pro');
        $ci->db->like('pro.Name',$keyword); 
...
function lookupProduct3($keyword,$store_id,$limit=10,$start=0){ 
        $ci = & get_instance();
        
        $ci->db->query('SET SQL_BIG_SELECTS=1');
        
        $ci->db->select('pro.*'); 
        $ci->db->from('estore_products ep');
        $ci->db->join('products as pro', 'ep.rsk_no = pro.RSKnummer AND ep.store_id = '.$store_id,'left');
        $ci->db->like('pro.Name',$keyword); 
        $ci->db->or_like('pro.RSKnummer',$keyword); 
        $ci->db->limit($limit,$start);
        $query = $ci->db->get();     
